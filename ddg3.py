...
def query(query, useragent=_useragent):
    ...
    url = "http://api.duckduckgo.com/?{}".format(params)
...
class Results(object):
    def __init__(self, xml):
        ...
        try:
            ...
            self.related = [Result(elem) for elem in xml.getiterator("RelatedTopic")]
...
def main():
    ...
    parser = OptionParser(
        usage="usage: %prog [options] query", version="ddg3 {}".format(__version__)
    )
    parser.add_option(
        "-o",
        "--open",
    ...
    if options.open:
        ...
        webbrowser.open(
            "http://duckduckgo.com/?{}".format(urllib.urlencode(dict(q=q)), new=2)
        )
